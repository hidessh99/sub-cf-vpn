...
async function getKVPrxList() {
    try {
        // 尝试从 GitHub 获取代理列表
        const response = await fetch("https://raw.githubusercontent.com/FoolVPN-ID/Nautica/refs/heads/main/kvProxyList.json");
        if (!response.ok) {
            console.warn("Failed to fetch KV proxy list from GitHub");
            return {};
        }
        const data = await response.json();
        return data || {};
    } catch (error) {
        console.error("Failed to fetch KV proxy list:", error);
        return {};
    }
}
            ...
            if (upgradeHeader === "websocket") {
                ...
                if (regionMatch) {
                    ...
                    if (REGION_MAP[regionKey] !== undefined) {
                        ...
                        console.log(`Selected Proxy (/${regionKey}${index !== null ? index + 1 : ""}): ${pxip}`);
                ...
                if (countryMatch) {
                    ...
                    console.log(`Selected Proxy (/${countryCode}${index !== null ? index + 1 : ""}): ${pxip}`);
                    globalThis.pxip = pxip;
                    return await websocketHandler(request);
                }

                // === 格式 /ip:port 或 /ip=port 或 /ip-port ===
                const ipPortMatch = path.match(/^\/(.+[:=-]\d+)$/);
                if (ipPortMatch) {
                    pxip = ipPortMatch[1].replace(/[=:-]/, ":");
                    console.log(`Direct Proxy IP: ${pxip}`);
                    globalThis.pxip = pxip;
                    return await websocketHandler(request);
                }
                ...
                if (url.pathname.length === 3 || url.pathname.includes(',')) {
                    ...
                    if (kvPrx[prxKey] && kvPrx[prxKey].length > 0) {
                        ...
                        console.log(`Legacy Proxy (/${prxKeys.join(",")}): ${pxip}`);
                        globalThis.pxip = pxip;
                        return await websocketHandler(request);
                    } else {
                ...
                // 如果没有匹配任何格式，直接使用原始路径中的 ip:port 格式
