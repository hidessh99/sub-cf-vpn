...
export class ProxyUseCase {
  ...
  async createProxy(data: CreateProxyRequest): Promise<ProxyIP> {
  ...
  async updateProxy(id: number, data: UpdateProxyRequest): Promise<ProxyIP> {
  ...
  async importFromJSON(list: ImportProxyItem[]): Promise<number> {
    ...
    const count = await this.proxyRepo.bulkCreate(formatted);
    return count;
  }

  async getPublicProxyList(): Promise<PublicProxyItem[]> {
    return await this.proxyRepo.getPublicList();
  }

  async getPublicProxyListGrouped(): Promise<Record<string, string[]>> {
